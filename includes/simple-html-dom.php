...
define('SEOPIC_HDOM_TYPE_ELEMENT', 1);
define('SEOPIC_HDOM_TYPE_COMMENT', 2);
define('SEOPIC_HDOM_TYPE_TEXT', 3);
define('SEOPIC_HDOM_TYPE_ENDTAG', 4);
define('SEOPIC_HDOM_TYPE_ROOT', 5);
define('SEOPIC_HDOM_TYPE_UNKNOWN', 6);
define('SEOPIC_HDOM_QUOTE_DOUBLE', 0);
define('SEOPIC_HDOM_QUOTE_SINGLE', 1);
define('SEOPIC_HDOM_QUOTE_NO', 3);
define('SEOPIC_HDOM_INFO_BEGIN', 0);
define('SEOPIC_HDOM_INFO_END', 1);
define('SEOPIC_HDOM_INFO_QUOTE', 2);
define('SEOPIC_HDOM_INFO_SPACE', 3);
define('SEOPIC_HDOM_INFO_TEXT', 4);
define('SEOPIC_HDOM_INFO_INNER', 5);
define('SEOPIC_HDOM_INFO_OUTER', 6);
define('SEOPIC_HDOM_INFO_ENDSPACE', 7);

defined('SEOPIC_DEFAULT_TARGET_CHARSET') || define('SEOPIC_DEFAULT_TARGET_CHARSET', 'UTF-8');
defined('SEOPIC_DEFAULT_BR_TEXT') || define('SEOPIC_DEFAULT_BR_TEXT', "\r\n");
defined('SEOPIC_DEFAULT_SPAN_TEXT') || define('SEOPIC_DEFAULT_SPAN_TEXT', ' ');
defined('SEOPIC_MAX_FILE_SIZE') || define('SEOPIC_MAX_FILE_SIZE', 600000);
define('SEOPIC_HDOM_SMARTY_AS_TEXT', 1);

function seopic_str_get_html(
    $str,
    $lowercase = true,
    $forceTagsClosed = true,
    $target_charset = SEOPIC_DEFAULT_TARGET_CHARSET,
    $stripRN = true,
    $defaultBRText = SEOPIC_DEFAULT_BR_TEXT,
    $defaultSpanText = SEOPIC_DEFAULT_SPAN_TEXT)
{
    $dom = new seopic_simple_html_dom(
        null,
        $lowercase,
        $forceTagsClosed,
        $target_charset,
    ...
    if (empty($str) || strlen($str) > SEOPIC_MAX_FILE_SIZE) {
        $dom->clear();
        return false;
    }
...
class seopic_simple_html_dom_node
{
    public $nodetype = SEOPIC_HDOM_TYPE_TEXT;
    public $tag = 'text';
    public $attr = array();
    ...
    function dump_node($echo = true)
    {
        ...
        if (isset($node->_[SEOPIC_HDOM_INFO_INNER])) {
            $string .= "'" . $node->_[SEOPIC_HDOM_INFO_INNER] . "'";
        } else {
            $string .= ' NULL ';
        }
    ...
    function innertext()
    {
        if (isset($this->_[SEOPIC_HDOM_INFO_INNER])) {
            return $this->_[SEOPIC_HDOM_INFO_INNER];
        }

        if (isset($this->_[SEOPIC_HDOM_INFO_TEXT])) {
            return $this->dom->restore_noise($this->_[SEOPIC_HDOM_INFO_TEXT]);
        }
    ...
    function outertext()
    {
        ...
        if (isset($this->_[SEOPIC_HDOM_INFO_OUTER])) {
            return $this->_[SEOPIC_HDOM_INFO_OUTER];
        }
        ...
        if ($this->dom && $this->dom->nodes[$this->_[SEOPIC_HDOM_INFO_BEGIN]]) {
            $ret = $this->dom->nodes[$this->_[SEOPIC_HDOM_INFO_BEGIN]]->makeup();
        }

        if (isset($this->_[SEOPIC_HDOM_INFO_INNER])) {
            // todo: <br> should either never have SEOPIC_HDOM_INFO_INNER or always
            if ($this->tag !== 'br') {
                $ret .= $this->_[SEOPIC_HDOM_INFO_INNER];
            }
        } elseif ($this->nodes) {
            foreach ($this->nodes as $n) {
        ...
        if (isset($this->_[SEOPIC_HDOM_INFO_END]) && $this->_[SEOPIC_HDOM_INFO_END] != 0) {
            $ret .= '</' . $this->tag . '>';
        }
    ...
    function text()
    {
        ...
        switch ($this->nodetype) {
            case SEOPIC_HDOM_TYPE_TEXT:
                return $this->dom->restore_noise($this->_[SEOPIC_HDOM_INFO_TEXT]);
            case SEOPIC_HDOM_TYPE_COMMENT:
                return '';
            case SEOPIC_HDOM_TYPE_UNKNOWN:
                return '';
        }
        ...
        // In rare cases, (always node type 1 or SEOPIC_HDOM_TYPE_ELEMENT - observed
        ...
        if (!is_null($this->nodes)) {
            foreach ($this->nodes as $n) {
                ...
                // after all.
                if ($n->tag === 'span') {
                    $ret .= $this->dom->SEOPIC_DEFAULT_SPAN_TEXT;
                }
            }
        }
        return $ret;
    ...
    function makeup()
    {
        ...
        foreach ($this->attr as $key => $val) {
            ...
            $ret .= $this->_[SEOPIC_HDOM_INFO_SPACE][$i][0];

            //no value attr: nowrap, checked selected...
            if ($val === true) {
                $ret .= $key;
            } else {
                switch ($this->_[SEOPIC_HDOM_INFO_QUOTE][$i]) {
                    case SEOPIC_HDOM_QUOTE_DOUBLE:
                        $quote = '"';
                        break;
                    case SEOPIC_HDOM_QUOTE_SINGLE:
                        $quote = '\'';
                        break;
                    default:
                        $quote = '';
                ...
                $ret .= $key
                    . $this->_[SEOPIC_HDOM_INFO_SPACE][$i][1]
                    . '='
                    . $this->_[SEOPIC_HDOM_INFO_SPACE][$i][2]
                    . $quote
                    . $val
                    . $quote;
            }
        ...
        return $ret . $this->_[SEOPIC_HDOM_INFO_ENDSPACE] . '>';
    }
    ...
    function find($selector, $idx = null, $lowercase = false)
    {
        ...
        for ($c = 0; $c < $count; ++$c) {
            ...
            if (!isset($this->_[SEOPIC_HDOM_INFO_BEGIN])) {
                return array();
            }

            $head = array($this->_[SEOPIC_HDOM_INFO_BEGIN] => 1);
            $cmd = ' '; // Combinator
    ...
    protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
    {
        ...
        if ($parent_cmd === ' ') { // Descendant Combinator
            ...
            $end = (!empty($this->_[SEOPIC_HDOM_INFO_END])) ? $this->_[SEOPIC_HDOM_INFO_END] : 0;
            if ($end == 0) {
                $parent = $this->parent;
                while (!isset($parent->_[SEOPIC_HDOM_INFO_END]) && $parent !== null) {
                    $end -= 1;
                    $parent = $parent->parent;
                }
                $end += $parent->_[SEOPIC_HDOM_INFO_END];
            }

            // Get list of target nodes
            $nodes_start = $this->_[SEOPIC_HDOM_INFO_BEGIN] + 1;
        ...
        foreach ($nodes as $node) {
            ...
            if ($pass) $ret[$node->_[SEOPIC_HDOM_INFO_BEGIN]] = 1;
            unset($node);
        }
    ...
    function __set($name, $value)
    {
        ...
        switch ($name) {
            case 'outertext':
                return $this->_[SEOPIC_HDOM_INFO_OUTER] = $value;
            case 'innertext':
                if (isset($this->_[SEOPIC_HDOM_INFO_TEXT])) {
                    return $this->_[SEOPIC_HDOM_INFO_TEXT] = $value;
                }
                return $this->_[SEOPIC_HDOM_INFO_INNER] = $value;
        }

        if (!isset($this->attr[$name])) {
            $this->_[SEOPIC_HDOM_INFO_SPACE][] = array(' ', '', '');
            $this->_[SEOPIC_HDOM_INFO_QUOTE][] = SEOPIC_HDOM_QUOTE_DOUBLE;
        }
...
class seopic_simple_html_dom
{
    ...
    protected $SEOPIC_DEFAULT_BR_TEXT = '';

    public $SEOPIC_DEFAULT_SPAN_TEXT = '';
        ...
        $options = 0)
    {
        ...
        if ($options & SEOPIC_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
        ...
        $this->parse();
        // end
        $this->root->_[SEOPIC_HDOM_INFO_END] = $this->cursor;
        $this->parse_charset();
        ...
        $defaultSpanText = SEOPIC_DEFAULT_SPAN_TEXT)
    {
        ...
        $this->SEOPIC_DEFAULT_BR_TEXT = $defaultBRText;
        $this->SEOPIC_DEFAULT_SPAN_TEXT = $defaultSpanText;
        $this->root = new seopic_simple_html_dom_node($this);
        $this->root->tag = 'root';
        $this->root->_[SEOPIC_HDOM_INFO_BEGIN] = -1;
        $this->root->nodetype = SEOPIC_HDOM_TYPE_ROOT;
    ...
    protected function parse()
    {
        while (true) {
            ...
            $node = new seopic_simple_html_dom_node($this);
            ++$this->cursor;
            $node->_[SEOPIC_HDOM_INFO_TEXT] = $s;
    ...
    protected function read_tag()
    {
        ...
        if ($this->char === '/') {
            ...
            if ($parent_lower !== $tag_lower) {
                ...
                if (isset($this->optional_closing_tags[$parent_lower])
                    ...
                    $this->parent->_[SEOPIC_HDOM_INFO_END] = 0;
                    ...
                    if (strtolower($this->parent->tag) !== $tag_lower) {
                        ...
                        $this->parent->_[SEOPIC_HDOM_INFO_END] = $this->cursor;
                    ...
                    $this->parent->_[SEOPIC_HDOM_INFO_END] = 0; // No end tag
                    $org_parent = $this->parent;
        ...
        // start tag
        $node = new seopic_simple_html_dom_node($this);
        $node->_[SEOPIC_HDOM_INFO_BEGIN] = $this->cursor;
        ++$this->cursor;
        ...
        if (isset($tag[0]) && $tag[0] === '!') {
            $node->_[SEOPIC_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

            if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
                $node->nodetype = SEOPIC_HDOM_TYPE_COMMENT;
                $node->tag = 'comment';
            } else { // Could be doctype or CDATA but we don't care
                $node->nodetype = SEOPIC_HDOM_TYPE_UNKNOWN;
                $node->tag = 'unknown';
            }

            if ($this->char === '>') {
                $node->_[SEOPIC_HDOM_INFO_TEXT] .= '>';
            }
        ...
        if ($pos = strpos($tag, '<') !== false) {
            $tag = '<' . substr($tag, 0, -1);
            $node->_[SEOPIC_HDOM_INFO_TEXT] = $tag;
            $this->link_nodes($node, false);
        ...
        if (!preg_match('/^\w[\w:-]*$/', $tag)) {
            $node->_[SEOPIC_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
        ...
        $node->nodetype = SEOPIC_HDOM_TYPE_ELEMENT;
        $tag_lower = strtolower($tag);
        ...
        do {
            ...
            if ($this->pos >= $this->size - 1 && $this->char !== '>') {
                $node->nodetype = SEOPIC_HDOM_TYPE_TEXT;
                $node->_[SEOPIC_HDOM_INFO_END] = 0;
                $node->_[SEOPIC_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
                $node->tag = 'text';
                $this->link_nodes($node, false);
                return true;
            }
            ...
            if ($this->doc[$this->pos - 1] == '<') {
                ...
                $node->attr = array();
                $node->_[SEOPIC_HDOM_INFO_END] = 0;
                $node->_[SEOPIC_HDOM_INFO_TEXT] = substr(
                    $this->doc,
                    $begin_tag_pos,
            ...
            if ($name !== '/' && $name !== '') { // this is a attribute name
                ...
                if ($this->char === '=') { // attribute with value
                    ...
                    //no value attr: nowrap, checked selected...
                    $node->_[SEOPIC_HDOM_INFO_QUOTE][] = SEOPIC_HDOM_QUOTE_NO;
                    $node->attr[$name] = true;
                    if ($this->char != '>') {
                ...
                $node->_[SEOPIC_HDOM_INFO_SPACE][] = $space;
        ...
        $this->link_nodes($node, true);
        $node->_[SEOPIC_HDOM_INFO_ENDSPACE] = $space[0];

        // handle empty tags (i.e. "<div/>")
        if ($this->copy_until_char('>') === '/') {
            $node->_[SEOPIC_HDOM_INFO_ENDSPACE] .= '/';
            $node->_[SEOPIC_HDOM_INFO_END] = 0;
        } else {
            // reset parent
        ...
        if ($node->tag === 'br') {
            $node->_[SEOPIC_HDOM_INFO_INNER] = $this->SEOPIC_DEFAULT_BR_TEXT;
        }
    ...
    protected function parse_attr($node, $name, &$space)
    {
        ...
        switch ($this->char) {
            case '"':
                $quote_type = SEOPIC_HDOM_QUOTE_DOUBLE;
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                $value = $this->copy_until_char('"');
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                break;
            case '\'':
                $quote_type = SEOPIC_HDOM_QUOTE_SINGLE;
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                $value = $this->copy_until_char('\'');
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                break;
            default:
                $quote_type = SEOPIC_HDOM_QUOTE_NO;
        ...
        if (!$is_duplicate) {
            $node->_[SEOPIC_HDOM_INFO_QUOTE][] = $quote_type;
    ...
    {
        return @seopic_str_get_html("<$name>$value</$name>")->firstChild();
    }

    function createTextNode($value)
    {
        return @end(seopic_str_get_html($value)->nodes);
    }
